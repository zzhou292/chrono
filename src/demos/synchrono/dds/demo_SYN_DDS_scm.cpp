...
int main(int argc, char* argv[]) {
    ...
    switch (node_id) {
        case 0:
            initLoc = ChVector3d(5, 0, 0.5);
            initRot = QuatFromAngleZ(CH_PI);  // Facing towards (0,0) from (3,0)
            curve_pts = {initLoc, initLoc - ChVector3d(100, 0, 0)};
            break;
        case 1:
            initLoc = ChVector3d(-5, 0, 0.5);
            initRot = QuatFromAngleZ(0);  // Facing towards (0,0) from (-3,0)
            curve_pts = {initLoc, initLoc + ChVector3d(100, 0, 0)};
            break;
        case 2:
            initLoc = ChVector3d(0, 5, 0.5);
            initRot = QuatFromAngleZ(-CH_PI / 2);  // Facing towards (0,0) from (0,3)
    ...
    // hmmwv.GetSystem()->SetSolverType(ChSolver::Type::APGD);
    ...
    auto vehicle_agent = chrono_types::make_shared<SynWheeledVehicleAgent>(
        &hmmwv.GetVehicle(), synchrono::GetDataFile("vehicle/col/HMMWV.json"));
    ...
    ChPathFollowerDriver driver(hmmwv.GetVehicle(), path, "Box path", 1);

    // Reasonable defaults for the underlying PID
    driver.GetSpeedController().SetGains(0.4, 0, 0);
    driver.GetSteeringController().SetGains(0.2, 0.05, 0);
...
void AddCommandLineOptions(ChCLI& cli) {
    ...
    cli.AddOption<double>("Simulation", "b,heartbeat", "Heartbeat", "1e-3");

    cli.AddOption<int>("DDS", "node_id", "ID for this Node", "1");
    cli.AddOption<int>("DDS", "num_nodes", "Number of Nodes", "2");

    // SCM specific options
    cli.AddOption<double>("Demo", "d,dpu", "Divisions per unit", "10");
