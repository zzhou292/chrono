...
room_mmesh.LoadWavefrontMesh(chrono.GetChronoDataFile("robot/environment/room_1/test1.obj"), False, True)
...
# ground = chrono.ChBodyEasyBox(60, 200, 1, 1000, True, True, ground_mat)
...
rover.Initialize(chrono.ChFrameD(chrono.ChVectorD(-4.5011, -17.773, 0.2), chrono.Q_from_AngAxis(chrono.CH_C_PI, chrono.ChVectorD(0, 0, 1))))
...
curve = chrono.ChBezierCurve()
path_file = chrono.GetChronoDataFile("robot/environment/room_1/my_curve.txt")
path = curve.read(path_file, True)
tracker = chrono.ChBezierCurveTracker(path)
