...
int main(int argc, char* argv[]) {
    ...
    double dynamics_percentage = (total_dynamics_step_time / total_elapsed_seconds) * 100.0;
    if (domain_manager.GetMPIrank() == 0) {
        ...
        std::cout << "Time spent in DoStepDynamics: " << std::fixed << std::setprecision(3) << total_dynamics_step_time
                  << " seconds (" << dynamics_percentage << "%)" << std::endl;
    }
