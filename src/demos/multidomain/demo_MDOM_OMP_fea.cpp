...
int main(int argc, char* argv[]) {
    ...
    domain_manager.AddDomain(domain_builder.BuildDomain(&sys_0,  // physical system of this domain
    ...
    domain_manager.AddDomain(domain_builder.BuildDomain(&sys_1,  // physical system of this domain
                                                        1        // rank of this domain
                                                        ));
    // Set the time stepper: we have FEA, use an explicit time stepper.
    auto explicit_timestepper1 = chrono_types::make_shared<ChTimestepperEulerExplIIorder>(&sys_1);
    explicit_timestepper1->SetConstraintsAsPenaltyON(
        2e6);  // use penalty for constraints, skip linear systems completely
    ...
    // Note that they have the same tag ID so that they are not transported through domain interfaces, and just
    // referenced.
    ...
    mrigidBody_floor_0->SetTag(unique_ID);
    unique_ID++;
