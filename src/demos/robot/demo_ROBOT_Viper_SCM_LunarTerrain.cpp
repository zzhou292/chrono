...
int main(int argc, char* argv[]) {
    ...
    while (vis->Run()) {
        ...
        if (output) {
            ...
            csv << sys.GetChTime() << viper.GetWheelAppliedTorque(ViperWheelID::V_LF)
                << viper.GetWheelAppliedTorque(ViperWheelID::V_RF) << viper.GetWheelAppliedTorque(ViperWheelID::V_LB)
                << viper.GetWheelAppliedTorque(ViperWheelID::V_RB) << std::endl;
        }
        ...
        if (step % 100 == 0) {
            ...
            std::cout << "RTF: " << static_cast<double>(duration.count()) / sys.GetChTime() / 1000 << std::endl;
        }
        step++;
    }
