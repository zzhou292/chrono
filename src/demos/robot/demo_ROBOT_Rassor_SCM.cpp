...
// =============================================================================
//
// Demo to show Rassor Rover operated on SCM Terrain
//
// =============================================================================
...
// Use custom material for the Rassor Wheel
...
int main(int argc, char* argv[]) {
    ...
    Rassor rover(&sys, wheel_type);

    rover.SetDriver(driver);
    if (use_custom_mat)
        rover.SetWheelContactMaterial(CustomWheelMaterial(ChContactMethod::NSC));

    rover.Initialize(ChFrame<>(ChVector<>(-5, 0, -0.2), QUNIT));

    // Get wheels and bodies to set up SCM patches
    auto Wheel_1 = rover.GetWheel(RassorWheelID::RA_LF)->GetBody();
    auto Wheel_2 = rover.GetWheel(RassorWheelID::RA_RF)->GetBody();
    auto Wheel_3 = rover.GetWheel(RassorWheelID::RA_LB)->GetBody();
    auto Wheel_4 = rover.GetWheel(RassorWheelID::RA_RB)->GetBody();
    auto Razor_1 = rover.GetRazor(RassorDirID::RA_F)->GetBody();
    auto Razor_2 = rover.GetRazor(RassorDirID::RA_B)->GetBody();
    ...
    if (enable_moving_patch) {
        ...
        terrain.AddMovingPatch(Razor_1, ChVector<>(0, 0, 0), ChVector<>(0.5, 2 * wheel_range, 2 * wheel_range));
        terrain.AddMovingPatch(Razor_1, ChVector<>(0, 0, 0), ChVector<>(0.5, 2 * wheel_range, 2 * wheel_range));
    }
    ...
    switch (vis_type) {
        case ChVisualSystem::Type::IRRLICHT: {
            ...
            vis_irr->SetWindowTitle("Rassor Rover on SCM");
            vis_irr->Initialize();
            vis_irr->AddLogo();
            vis_irr->AddSkyBox();
        ...
        case ChVisualSystem::Type::VSG: {
            ...
            vis_vsg->SetWindowTitle("Rassor Rover on SCM");
    ...
    while (vis->Run()) {
        ...
        if (output) {
            ...
            csv << sys.GetChTime() << rover.GetWheelTracTorque(RassorWheelID::RA_LF)
                << rover.GetWheelTracTorque(RassorWheelID::RA_RF) << rover.GetWheelTracTorque(RassorWheelID::RA_LB)
                << rover.GetWheelTracTorque(RassorWheelID::RA_RB) << std::endl;
        }
