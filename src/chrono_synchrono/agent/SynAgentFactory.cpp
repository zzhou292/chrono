...
std::shared_ptr<SynAgent> SynAgentFactory::CreateAgent(std::shared_ptr<SynMessage> description) {
    ...
    if (auto wv_description = std::dynamic_pointer_cast<SynWheeledVehicleDescriptionMessage>(description)) {
        ...
        agent = copter_agent;
    } else if (auto robot_description = std::dynamic_pointer_cast<SynRobotDescriptionMessage>(description)) {
        auto robot_agent = chrono_types::make_shared<SynRobotAgent>();
        robot_agent->SetKey(source_key);
