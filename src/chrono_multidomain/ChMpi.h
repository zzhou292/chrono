...
class ChApiMultiDomain ChMPI {
  public:
    ...
    enum eCh_mpiCommMode { MPI_STANDARD = 0, MPI_BUFFERED, MPI_SYNCHRONOUS, MPI_READY };

    /// Send a ChMatrix to the process with rank destID.
    /// Blocking version, wrapping MPI_Send, MPI_Bsend, MPI_Rsend, MPI_Ssend   modes.
    static int SendMatrix_blocking(int destID,                            ///< destination rank
    ...
    static int SendMatrix_nonblocking(int destID,                            ///< destination rank
                                      ChMatrixDynamic<double>& source_matr,  ///< source matrix
                                      eCh_mpiCommMode mmode,                 ///< send mode
    ...
    /// Blocking version.
    static int SendString_blocking(int destID,               ///< destination rank
    ...
    static int SendString_nonblocking(int destID,               ///< destination rank
                                      std::string& source_str,  ///< source string
                                      eCh_mpiCommMode mmode,    ///< send mode
                                      ChMPIrequest* mreq        ///< if nonblocking, must use this
    );
    static int SendString_nonblocking2(int destID,               ///< destination rank
                                       std::string& source_str,  ///< source string
                                       eCh_mpiCommMode mmode,    ///< send mode
                                       ChMPIrequest* mreq        ///< if nonblocking, must use this
    );
    ...
    static int ReceiveString_blocking(int sourceID,           ///< source rank
                                      std::string& dest_str,  ///< destination string - will be resized
                                      ChMPIstatus* mstatus    ///< return status here
    );

    static int ReceiveString_blocking2(int sourceID,           ///< source rank
                                       std::string& dest_str,  ///< destination string - will be resized
                                       ChMPIstatus* mstatus);
    ...
    enum class eCh_mpiReduceOperation { MPI_max = 0, MPI_min, MPI_sum, MPI_prod };
    static int ReduceAll(double send,
                         double& received_result,
                         eCh_mpiReduceOperation operation = eCh_mpiReduceOperation::MPI_sum);

    /// Get rank of this process
    static int GetRank();

    /// Broadcast a vector of objects from root rank to all other ranks
    template <typename T>
