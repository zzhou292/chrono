...
void InterfaceManageNodeSharedLeaving(std::shared_ptr<T_node_serial> obj,
...
void ChDomain::DoUpdateSharedLeaving() {
    ...
    for (auto& interf : this->interfaces) {
        ...
        serializer->SetUseGetTagAsID(
            true);  // GetTag of items, when available, will be used to generate unique IDs in serialization

        // Prepare a map of pointers to shared items so that they are not serialized but just referenced
        // by tag. For this reason we traverse all the pointers in the shared items, and ChArchivePointerMap will
        // capture their tags. Note that these shared items might contain pointers to auxiliary structures like visual
        // models, contact materials etc.; if these structures have a GetTag(), the ChArchivePointerMap will capture
        // these IDs too, which is good because if the shared nodes are also in the neighbouring domain, so will be also
        // those aux structures
        ...
        /*
        std::cout << "           domain << " << this->rank << " ExternalPointersMap size: " <<
        rebinding_pointers.pointer_map_ptr_id.size() << "\n"; for (auto& m : rebinding_pointers.pointer_map_ptr_id)
        ...
        *serializer << CHNVP(shared_ids, "shared_ids");
        ...
        // std::cout << "\nSERIALIZE domain " << this->GetRank() << " to interface " <<
        // interf.second.side_OUT->GetRank() << "\n"; //***DEBUG std::cout << interf.second.buffer_sending.str();
        // //***DEBUG
    }
}
    ...
    if (delete_outsiders) {
        ...
        for (const auto& mmesh : system->GetMeshes()) {
            ...
            for (const auto& delelement : elements_to_remove)
                mmesh->RemoveElement(
                    delelement);  // can be made more efficient - this require N searches in vector container
        ...
        for (const auto& delitem : items_to_remove)
            system->RemoveOtherPhysicsItem(
                delitem);  // can be made more efficient - this require N searches in vector container

        std::vector<std::shared_ptr<chrono::ChBody>> bodies_to_remove;
        for (const auto& body : system->GetBodies()) {
            auto mabb = body->GetTotalAABB();
            bool is_overlapping_IN =
                this->IsOverlap(mabb) || this->IsInto(body->GetPos());  // if aabb is empty (reversed) use center
