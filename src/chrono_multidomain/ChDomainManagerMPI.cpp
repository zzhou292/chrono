...
bool ChDomainManagerMPI::DoDomainSendReceive(int mrank) {
    ...
    if (!(domain->IsMaster() && !this->master_domain_enabled)) {
        for (auto& minterface : domain->GetInterfaces()) {
            ...
            // cannot be used directly for MPI send because it is a std::stringstream, that has no accessible buffer
            // (maybe noncontiguous)
            send_strings[i_int] = minterface.second.buffer_sending.rdbuf()->str();
            mpi_engine.SendString_nonblocking(other_rank, send_strings[i_int], ChMPI::eCh_mpiCommMode::MPI_STANDARD,
                                              &requests[i_int]);
        ...
        // receiving stuff below, aiming at highest performance with high communication overlap. Sadly, not possible as
        // ReceiveString is only available in blocking version, because of the problem with receiving buffer size not
        // known in advance.
...
int ChDomainManagerMPI::ReduceAll(int mrank,
                                  double send,
    ...
    switch (operation) {
        case eCh_domainsReduceOperation::max:
            mpi_operation = ChMPI::eCh_mpiReduceOperation::MPI_max;
            break;
        case eCh_domainsReduceOperation::min:
            mpi_operation = ChMPI::eCh_mpiReduceOperation::MPI_min;
            break;
        case eCh_domainsReduceOperation::sum:
            mpi_operation = ChMPI::eCh_mpiReduceOperation::MPI_sum;
            break;
        case eCh_domainsReduceOperation::prod:
            mpi_operation = ChMPI::eCh_mpiReduceOperation::MPI_prod;
            break;
    }
