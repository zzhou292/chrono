...
ANCFBeamTest::ANCFBeamTest(bool useContInt) {
    ...
    m_system->SetGravitationalAcceleration(ChVector3d(0, 0, 0.0));
    ...
    double k1 = 1.0;  // No shear correction (instead of 10*(1+nu)/(12+11*nu))
    double k2 = 1.0;  // No shear correction
...
bool ANCFBeamTest::MassMatrixCheck(int msglvl) {
    ...
    // After building the system matrix
    std::cout << "Global Mass Matrix (formatted):\n";
    std::cout << std::scientific;       // Use scientific notation for better formatting
    std::cout << std::setprecision(4);  // Set precision to 4 decimal places

    // Add a header for columns
    std::cout << std::setw(6) << "Row" << std::setw(6) << "Col" << std::setw(20) << "Value" << std::endl;
    ...
    for (int i = 0; i < MassMatrix.rows(); ++i) {
        for (int j = 0; j < MassMatrix.cols(); ++j) {
            double val = MassMatrix.coeff(i, j);
            if (std::abs(val) > 1e-12) {  // Only print non-zero values
                std::cout << std::setw(6) << i << std::setw(6) << j << std::setw(20) << val << std::endl;
            }
        }
    }
...
bool ANCFBeamTest::GeneralizedInternalForceSmallDispNoVelCheck(int msglvl) {
    ...
    // m_nodeB->SetSlope1(ChVector3d(1.0034, 0.0, 0.0));
    m_nodeB->SetSlope3(ChVector3d(0.0, 0.0, 0.9923));
    ...
    std::cout << "InternalForceSmallDispNoVel = " << InternalForceSmallDispNoVel << std::endl;

    // Add the GQ table printout here
    std::cout << "\n=== Gauss Quadrature Points and Weights ===\n";
    chrono::ChQuadratureTables* tables = chrono::ChQuadrature::GetStaticTables();
    tables->PrintTables();
    std::cout << "\n-------------------------------------\n";
...
bool ANCFBeamTest::GeneralizedInternalForceNoDispSmallVelCheck(int msglvl) {
    ...
    m_nodeB->SetPosDt(ChVector3d(0.0, 0.0, 0.0));
...
bool ANCFBeamTest::JacobianSmallDispNoVelNoDampingCheck(int msglvl) {
    ...
    // Setup the test conditions
    ChVector3d OriginalPos = m_nodeB->GetPos();
    m_nodeB->SetPos(ChVector3d(m_nodeB->GetPos().x(), m_nodeB->GetPos().y(), 0.00256));
...
bool ANCFBeamTest::CantileverTipLoadCheck(int msglvl) {
    ...
    // Mesh properties - Dimensions and material from the Princeton Beam Experiment Addendum
    int num_elements = 2;
    double length = 4.0;  // in->m
    double width = 1.0;   // in->m
    double height = 1.0;  // in->m
    // Aluminum 7075-T651 Material Properties
    double rho = 2700;  // kg/m^3
    double E = 7e8;     // Pa
    double nu = 0.33;
    ...
    class MyLoaderTimeDependentTipLoad : public ChLoaderUatomic {
      public:
        ...
        virtual void ComputeF(double U,                    // normalized position along the beam axis [-1...1]
            ...
            double t_end = 0.02;  // 100 steps * 0.0001 s/step

            if (t <= t_end) {
                F(0) = TIP_FORCE;  // Apply the force along the global Z axis (beam axis)
            }
        }
    ...
    for (int i = 0; i < 400; i++) {
        ...
        double displacement = point.x() - 4.0;
    ...
    std::cout << "Displacement_Model: " << Displacement_Model << std::endl;
    // std::cout << "Displacement_Theory: " << Displacement_Theory << std::endl;
